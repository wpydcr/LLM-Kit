...
class claude:
    ...
    def setv(self, cookie = ''):
        if cookie == '':
            return {
                'status': -1,
                'message': '请输入claude2网页cookie'
            }
        self.cookie = cookie 
        self.organization_id = self.get_organization_id()
        conversation = self.create_new_chat()
        self.conversation_id = conversation['uuid']
        return {
            'status': 0,
            'message': '设置成功'
        }

    def talk(self, prompt, stream = False, attachment = None):
        url = "https://claude.ai/api/append_message"
        # Upload attachment if provided  -- haven't implement
        attachments = []
        if attachment:
    ...
    def upload_attachment(self, file_path):
        ...
        response = requests.post(url, headers = headers, data = files, impersonate = "chrome110")
