...
def data_page():
    with gr.Tab("LLM数据集制作"):
        with gr.Row():
            with gr.Column(scale=4):
                question1=gr.Textbox(lines=3, placeholder="Write Here...",label="问题：")
                answer1=gr.Textbox(lines=5, placeholder="Write Here...",label="答案：")
                with gr.Row():
                    save1 = gr.Button("保存", variant="primary")
                    llm_sikp = gr.Button('跳过')
                    gpt1 = gr.Button("单次gpt生成")
                    start2 = gr.Button("批量gpt生成")
                    stop2 = gr.Button("停止gpt生成")
                    emptyBtn10 = gr.Button("清空json")
                    emptyBtn11 = gr.Button("清空上传")
                    back1 = gr.Button("撤回")
                json_d1=gr.JSON(data_pro.json_dict,label="已保存数据")
            with gr.Column(scale=1):
                    upload2 = gr.File(label="上传问题集(txt格式)",file_count="single",file_types=['.txt'])
                    with gr.Accordion('gpt参数',open=False):
                        openai_prompt=gr.Textbox(value="这是一个能够将文本翻译成中文的AI助手。请将引号中的文本翻译成简体中文。",lines=3, placeholder="Write Here...",label="提示词")
                        ...
                        timesleep = gr.Slider(0, 21, value=0.02, step=0.01, label="批量任务时间间隔（秒）:", interactive=True)
                    with gr.Accordion('下载数据',open=False):
                        download_path1=gr.Textbox(lines=1, placeholder="Write Here...",label="保存文件名:")
                        split_data11 = gr.Checkbox(label='是否划分验证集')
                        download11 = gr.Button("下载数据", variant="primary")
    with gr.Tab("文档转LLM数据集"):
        with gr.Row():
            with gr.Column(scale=4):
                doc3=gr.Textbox(label="语料：")
                with gr.Row():
                    gpt3 = gr.Button("单次gpt生成", variant="primary")
                    start3 = gr.Button("批量gpt生成")
                    stop3 = gr.Button("停止gpt生成")
                    emptyBtn30 = gr.Button("清空json")
                    emptyBtn31 = gr.Button("清空上传")
                    back3 = gr.Button("撤回")
                json_d3=gr.JSON(data_pro.json_dict,label="已保存数据")
                org_json_d3=gr.JSON(data_pro.json_dict,label="原始数据")
            with gr.Column(scale=1):
                    upload3 = gr.File(label="上传文档(txt格式)",file_count="single",file_types=['.txt'])
                    with gr.Accordion('gpt参数',open=False):
                        openai_prompt3=gr.Textbox(value="这是一个能够根据文本内容，生成相关问答数据集的AI助手。请根据引号中的文本提炼出5个相关问题和回答。",lines=3, placeholder="Write Here...",label="提示词")
                        ...
                        timesleep3 = gr.Slider(0, 21, value=0.02, step=0.01, label="批量任务时间间隔（秒）:", interactive=True)
                    with gr.Accordion('下载数据',open=False):
                        download_path3=gr.Textbox(lines=1, placeholder="Write Here...",label="保存文件名:")
                        split_data31 = gr.Checkbox(label='是否划分验证集')
                        download33 = gr.Button("下载原始数据")
                        download31 = gr.Button("下载数据", variant="primary")
    with gr.Tab("LLM数据集转换"):
        question2=gr.Textbox(lines=1, label="问题key",value='instruction')
        answer2=gr.Textbox(lines=1, label="回答key",value='output')
        models2 = gr.Radio(['json(格式："[{},{}]")','jsonl(格式："{}{}")'], label="*数据集格式",value='jsonl')
        models3 = gr.Radio(['新建','合并'], label="上传方式",value='新建')
        upload1 = gr.File(label="上传数据集",file_count="single",file_types=['.json','.jsonl','.data'])
        emptyBtn2 = gr.Button("清空上传")
        out=gr.JSON(data_pro.upload_dict,label="预览后6行")
        download_path2=gr.Textbox(lines=1, placeholder="Write Here...",label="保存文件名:")
        split_data21 = gr.Checkbox(label='是否划分验证集')
        download21 = gr.Button("下载数据", variant="primary")
    with gr.Tab("Embedding数据集制作"):
        with gr.Row():
            with gr.Column(scale=4):
                sentence1=gr.Text(label="语料1:")
                sentence2=gr.Text(label="语料2:")
                with gr.Row():
                    embed_relate = gr.Button(value="相关",variant="primary")
                    embed_unrelate = gr.Button(value="不相关",)
                    embed_batch_generate = gr.Button("批量gpt生成")
                    embed_stop_generate = gr.Button("停止gpt生成")
                    embed_empty_json = gr.Button("清空json")
                    embed_empty_upload = gr.Button("清空上传")
                    embed_back = gr.Button("撤回")
                embed_json_dict=gr.JSON(data_pro.embedding_json_dict,label="已保存数据")
            with gr.Column(scale=1):
                 with gr.Accordion(label='上传数据集(txt格式)'):
                    embed_upload = gr.UploadButton("上传数据集",file_types=['text'])
                 with gr.Accordion('gpt参数',open=False):
                    openai_prompt_embed = gr.Textbox(value="这是一个能够判断两句话是否相关的AI助手。请判断引号中的两句话是否相关。如果相关输出1，不相关输出0，注意：只需要输出0或者1，不需要输出其他任何内容。",lines=3, placeholder="Write Here...",label="提示词")
                    ...
                    timesleep_embed = gr.Slider(0, 100, value=0.02, step=0.01, label="批量任务时间间隔（秒）:", interactive=True)
                 with gr.Accordion('下载数据',open=False):
                    embed_download_path=gr.Textbox(lines=1, placeholder="Write Here...",label="数据文件夹名称：")
                    train_valid = gr.Checkbox(label='划分训练验证集')
                    embed_download = gr.Button("下载数据",variant='primary')
                    embed_upload_ok = gr.Textbox(lines=10, label="处理结果")
    with gr.Tab('Embedding数据集转换'):
        embed_exchange_upload = gr.File(label="上传数据集",file_count="single",file_types=['.data','.txt'])
        embed_exchange_emptyBtn = gr.Button("清空上传")
        embed_exchange_models = gr.Radio(['新建','合并'], label="上传方式",value='新建')
        embed_exchange_out=gr.JSON(data_pro.upload_dict,label="预览后6行")
        embed_exchange_download_path=gr.Textbox(lines=1, placeholder="Write Here...",label="保存文件名:")
        embed_exchange_train_valid = gr.Checkbox(label='划分训练验证集')
        embed_exchange_download = gr.Button("下载数据",variant='primary')
    with gr.Tab("知识库"):
        with gr.Tab("FAISS"):
            with gr.Tab("创建新的知识库"):
                vs_name = gr.Textbox(lines=1, label="库名")
                ...
                emb_refresh = gr.Button("刷新")
                files = gr.File(label="添加文件(支持md、pdf、txt、csv格式)", file_count="multiple")
                upload = gr.Button("生成知识库", variant="primary")
                upload_ok = gr.Textbox(lines=10, label="处理结果")
            with gr.Tab("查询已有数据库内的文档"):

                doc0 = gr.Radio(docs, label="*本地的向量知识库", value=None)
                with gr.Tab('Embedding API'):
                    create_emb_api_list = gr.Radio(embedding_api, label="", value=None)
                    with gr.Accordion('openai参数', open=True, visible=False) as create_openai_params:
                        create_openai_api_key = gr.Textbox(
                            lines=1, placeholder="Write Here...", label="*openai_api_key:", type='password')
                        create_openai_port = gr.Textbox(
                            lines=1, value='', label="*VPN proxyPort:")
                    with gr.Accordion('azure openai参数', open=True, visible=False) as create_azure_openai_params:
                        ...
                        create_azure_engine = gr.Textbox(
                            lines=1, value='', label="*engine:(azure openai)")
                with gr.Tab('Embedding Model'):
                    create_emb_model_list = gr.Radio(embs, label="", value=None)
                refresh_edit = gr.Button("刷新")
                search_button = gr.Button("查询",variant="primary")
                file_in_doc = gr.CheckboxGroup([], label="库中的文档", value=None)
                delete_selected_file = gr.Button("从库中删除选定的文档")
                add_new_file = gr.File(label="添加文件(支持md、pdf、txt、csv格式)", file_count="multiple")
                add_button = gr.Button("向库中添加文档")

        with gr.Tab("MySQL"):
            # 数据库地址、端口、用户名、密码、数据库名称
            host = gr.Textbox(lines=1, label="*数据库地址", value='localhost')
            user = gr.Textbox(lines=1, label="*用户名", value='root')
            password = gr.Textbox(lines=1, label="*密码",type= 'password', value='123456')
            port = gr.Textbox(lines=1, label="*端口", value='3306')
            connect = gr.Button("连接数据库", variant="primary")
            database = gr.Radio(label="选择数据库",value=None)
            new_database = gr.Textbox(lines=1, label="新建数据库", placeholder="Write Here...", visible=False)
            # 上传文件，支持多文件上传，文件类型为csv、txt、excel
            mysql_files = gr.File(label="添加文件(支持txt、csv、excel格式)",file_count="multiple")
            # 上传按钮
            mysql_upload = gr.Button("生成知识库")
            # 上传结果
            mysql_upload_ok=gr.Textbox(lines=10, label="处理结果")
        # with gr.Tab("other"):
        #     pass
    ...
    download11.click(data_pro.dl_jsonl1, [download_path1,split_data11],[],show_progress=True)
    ...
    download31.click(data_pro.dl_jsonl1, [download_path3,split_data31],[],show_progress=True)
    download33.click(data_pro.dl_jsonl1,[download_path3,split_data31],[],show_progress=True)
    ...
    download21.click(data_pro.dl_jsonl2, [download_path2,split_data21],[],show_progress=True)
    ...
    embed_exchange_download.click(data_pro.dl_embed,[embed_exchange_download_path,embed_exchange_train_valid],outputs=[],show_progress=True)
